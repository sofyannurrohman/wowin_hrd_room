...
func (r *UserRepository) Create(ctx context.Context, u *domain.User) error {
	query := `INSERT INTO users (id, role_id, name, email, password_hash, created_at)
	          VALUES ($1, $2, $3, $4, $5, $6)`
	_, err := r.db.Exec(ctx, query, u.ID, u.RoleID, u.Name, u.Email, u.PasswordHash, time.Now())
	return err
}

func (r *UserRepository) FindByEmail(ctx context.Context, email string) (*domain.User, error) {
	query := `SELECT u.id, u.role_id, r.name, u.name, u.email, u.password_hash, u.created_at
	          FROM users u JOIN roles r ON u.role_id = r.id WHERE u.email = $1`
	row := r.db.QueryRow(ctx, query, email)
	u := &domain.User{}
	err := row.Scan(&u.ID, &u.RoleID, &u.RoleName, &u.Name, &u.Email, &u.PasswordHash, &u.CreatedAt)
	if err != nil {
...
func (r *UserRepository) List(ctx context.Context) ([]domain.User, error) {
	query := `SELECT u.id, u.role_id, r.name, u.name, u.email, u.created_at
	...
	for rows.Next() {
		var u domain.User
		if err := rows.Scan(&u.ID, &u.RoleID, &u.RoleName, &u.Name, &u.Email, &u.CreatedAt); err != nil {
			return nil, err
		}
